...
namespace dev_locked
{
    struct screen
    {
        screen(hosting::local::host &localhost) : localhost{localhost} {}
        void render(ignacionr::cppgpt &gpt)
        {
            hs.render(hosting::ssh::host::by_name("dev-locked"), localhost);
            if (ImGui::CollapsingHeader("Status"))
            {
                ...
                views::Assertion("The Git service container is running", [this]
                                 { return hosting::ssh::host::by_name("dev-locked")->docker().is_container_running("gitservice", localhost); });
                ImGui::Indent();
                ImGui::Unindent();
                views::Assertion("The externalizer service is running", [this]
                                 { return hosting::ssh::host::by_name("dev-locked")->docker().is_container_running("socat-proxy", localhost); });
                ImGui::Indent();
                ImGui::Unindent();
                views::Assertion("The pipeline runner service is running", [this]
                                 { return hosting::ssh::host::by_name("dev-locked")->docker().is_container_running("pipeline-runner", localhost); });
                ImGui::Indent();
                views::Assertion("root on the pipeline runner container can log in for gitservice usage with ssh", [this]
                                 {
                        auto const result = hosting::ssh::host::by_name("dev-locked")->docker().execute_command("ssh -T git@gitservice", "pipeline-runner", localhost, false);
                        return result.find("Welcome to git-server-docker!") != std::string::npos; });
                ImGui::Unindent();
                views::Assertion("staging deployment is running", [this]
                                 { return hosting::ssh::host::by_name("dev-locked")->docker().is_container_running("foxbetbackend", localhost); });
                ImGui::SameLine();
            ...
            if (ImGui::CollapsingHeader("Tools"))
            {
                ...
                {
                    seed_result = hosting::ssh::host::by_name("dev-locked")->docker().execute_command("bash -c 'cd /home/ubuntu/arangodb-infra/dev-locked && ./seed.sh'", localhost, false);
                }
